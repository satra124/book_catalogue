...
def add_entry():
    list1.delete(0, END)
    dab.insert(title_text.get(), year_text.get(),
...
def delete_entry():
    list1.delete(0, END)
    id = dab.get_id(selected_row[0], selected_row[1],
...
def update_entry():
    ...
    id = dab.get_id(selected_row[0], selected_row[1],
                    selected_row[2], selected_row[3])
    dab.update(id, title_text.get(), year_text.get(),
               author_text.get(), isbn_text.get())
    search_entry()
...
b5 = Button(window, text="Update Entry", command=update_entry)
